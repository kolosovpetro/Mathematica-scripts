...
Pow[x_, s_] := x^(Boole[Mod[s, 2]==0]) * x^(2 Floor[(s-1)/2] + 1);
...
P2[m_, n_, b_] := n^Boole[Mod[m, 2]==0]* P[Floor[(m-1)/2], n, b];
...
ConvPowerIdentityParametric[m_, x_, a_] := Refine[Sum[CoeffA[m,r] * MacaulayDiscConv[x, r, a], {r, 0, m}], Element[x, Integers], Assumptions -> x>2*a];
